function renderWorkPackage({ taskDescription, product, architecture, qa, prompt, release }) {
  return [
    ...
    `- Product Strategist: ${product.activitySummary || product.valueStatement || product.objective}`,
    `- System Architect: ${architecture.activitySummary || architecture.boundaryStatement || architecture.objective}`,
    `- QA Sentinel: ${qa.activitySummary || qa.acceptanceStatement || qa.objective}`,
